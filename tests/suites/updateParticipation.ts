...
export const testUpdateParticipation = (provider: Provider) =>
  describe("Update a participation", () => {
    ...
    it("Update a participation", async () => {
      const [lotteryAddress, lotteryBump] = await PublicKey.findProgramAddress(
        [Buffer.from("lottery"), lotteryKey.toBuffer()],
        program.programId
      );
      ...
      expect(r.spendings.map((e) => e.toString()).toString()).to.equal(
        spendings.map((e) => e.mul(new BN(2)).toString()).toString()
      );
